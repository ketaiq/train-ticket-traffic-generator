...
class IrregularBudgetRequest(PassengerRequest):
    ...
    def perform_actions(self, logger_tasks):
        ...
        sleep(randint(1, 2))
        self.tickets_search(randint(1, 1), randint(5, 10))

        sleep(randint(1, 2))
        self.gen_ticket_info(False, False, False)
        ...
        # Delete the passenger's contact
        # from ts.services.contacts_service import delete_one_contact_request
        # sleep(randint(1, 2))
        # contact_id = delete_one_contact_request(self.request_id, self.admin_bearer, self.contact_id)
